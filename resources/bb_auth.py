...
def get_access_refresh_tokens(auth_code):
    ...
    if codes.status_code == 200:
        response_json = codes.json()
        access_token = response_json.get('access_token')
        refresh_token = response_json.get('refresh_token')

        # Print the new tokens to the terminal
        print(f"Access Token: {access_token}")
        print(f"Refresh Token: {refresh_token}")
        print(f"Full Response: {response_json}")

        # Update .ini file with new tokens
        if access_token and refresh_token:
            config['tokens']['access_token'] = access_token
            config['tokens']['refresh_token'] = refresh_token
            with open(config_file_name, 'w') as configfile:
                config.write(configfile)
            print("Tokens updated successfully in the .ini file.")
        else:
            print("Failed to retrieve tokens from the response.")
    else:
        print(f"Error fetching tokens: {codes.status_code}")
        print(codes.text)
